...
- Treat GitHub URLs inside the task as context only. Do not claim that a URL was reviewed unless it matches `real_workspace`; v1.0.2 does not auto-clone prompt URLs.
...
- On `SNAPSHOT_TOO_LARGE`, surface `snapshot_mb`, `snapshot_limit_mb`, and `largest_snapshot_roots`; recommend `.agysafeignore` or a smaller workspace before increasing limits.
